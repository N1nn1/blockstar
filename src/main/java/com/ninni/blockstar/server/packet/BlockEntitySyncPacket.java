...
public class BlockEntitySyncPacket {
    ...
    public static void encode(BlockEntitySyncPacket object, FriendlyByteBuf buffer) {
        buffer.writeBlockPos(object.pos);
        buffer.writeNbt(object.tag);
    }

    public static BlockEntitySyncPacket decode(FriendlyByteBuf buffer) {
    ...
    public static void handle(BlockEntitySyncPacket packet, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (ctx.get().getDirection() == NetworkDirection.PLAY_TO_CLIENT)
                world = Blockstar.PROXY.getWorld();
            else {
